...
import {divideBnToNumber, floorToDecimal, getTokenMultiplierFromDecimals, sleep, useLocalStorageState,} from './index';
    ...
    const markets: Array<{
      ...
      getMarketInfos(customMarkets).map(async (marketInfo) => {
        try {
          ...
          notify({
            title: 'Error loading all market',
...
export function useOpenOrdersAccounts(fast = false) {
  ...
  return useAsyncData<OpenOrders[] | null>(
    ...
    tuple('getOpenOrdersAccounts', wallet, market, wallet?.connected),
...
export function useFeeDiscountKeys(): [
  ...
  return useAsyncData(
    getFeeDiscountKeys,
    tuple('getFeeDiscountKeys', wallet, market, wallet?.connected),
    { refreshInterval: _SLOW_REFRESH_INTERVAL },
  );
}
...
export const useAllOpenOrders = (): {
  ...
  }, [connection, wallet?.connected, wallet, refresh]);
  return {
...
export function useWalletBalancesForAllMarkets(): {
  ...
  if (!wallet?.connected || !mintInfosConnected) {
    return [];
  }
  ...
  async function getOpenOrdersForDeprecatedMarkets() {
    ...
    const getOrders = async (market: Market | null) => {
      ...
      try {
        ...
        notify({
            title: `Error loading open orders for deprecated ${marketName}`,
          description: e.message,
          type: 'error',
        });
        return null;
